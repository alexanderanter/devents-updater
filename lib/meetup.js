...
const getEvents = async (config) => {
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      data = await apiPromise({ text: 'blockchain', page: 200, text_format: 'plain' });
    } catch(err) {
      pino.error(err.message);
      return Promise.resolve([]);
    }
    ...
    while (data.meta.next) {
      events.push(...data.results.map(async event => {
        ...
        const city = event.venue.city.replace(/\d+(\s|)\d+/, '').trim(); // remove numbers in city, ex: 111 22 stochkolm.
      ...
      try {
        data = await apiPromise({ text: 'blockchain', page: 200, text_format: 'plain', offset });
      } catch(err) {
        pino.error(err.message);
        return Promise.resolve([]);
      }
