...
const getEvents = (config) => {
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      data = await apiPromise({ q: 'blockchain', expand: 'venue' });
    } catch(err) {
      pino.error(err.message);
      return Promise.resolve([]);
    }
    ...
    while(data.pagination.has_more_items) {
      events.push(...data.events.map(async event => {
        ...
        // Bail if no venue.
        if (!event.venue || typeof event.venue.address === 'undefined') {
          return;
        }
  
        // Bail if bad city name and if region is not a string.
        if (typeof event.venue.address.city !== 'string') {
          if (typeof event.venue.address.region !== 'string') {
            return;
          }
          ...
          if (!/[A-Z]/.test(event.venue.address.city[0])) {
            return;
          }
        }
        ...
        return {
          ...
          city: event.venue.address.city.split(',')[0], // Only city name needed.
          link: event.url,
      ...
      try {
        data = await apiPromise({ q: 'blockchain', page, expand: 'venue' });
      } catch(err) {
        pino.error(err.message);
        return Promise.resolve([]);
      }
